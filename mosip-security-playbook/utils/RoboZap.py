...
class RoboZap(object):
    ...
    def zap_start_ascan(self, contextid, url, policy="Default Policy"):
        """
        ...
        try:
            scan_id = self.zap.ascan.scan(
                contextid=contextid, url=url, scanpolicyname=policy
            )
            time.sleep(5)
            while self.zap.ascan.status() < 100:
                logger.info("Ascan progress %: " + self.zap.ascan.status())
                time.sleep(5)
            logger.info("Ascan Complete")
            return scan_id
        except Exception as e:
            print(e.message)
